...
function buildPairSection7_PairSynthesis(
  ...
  if (hasHarmony && !hasClash) {
    parts.push("你们的合盘在多个维度上呈现出天然的协调性——这不是偶然的'缘分好'，而是双方八字结构层面的契合。五行层面，彼此的用神与日主存在正向互动，日柱关系倾向于合而非冲，这意味着日常相处中的摩擦成本天然偏低，感情的'维护成本'低于大多数伴侣。");
    parts.push("");
    parts.push("但专业合盘需要特别指出一点：太合的盘面有时会让人产生'不需要经营'的错觉。实际上，高度契合的伴侣反而容易因为缺少外部提醒而忽略关系中的小问题——就像一栋地基很好的房子，如果长期不检修，细微的裂缝也会慢慢扩大。建议你们有意识地为关系设置'成长节点'：每年至少一次深度的共同反思，讨论彼此的需求变化和下一步的共同目标。高匹配度是起点而非终点，保持新鲜感和共同成长才是长期稳定的真正关键。");
  } else if (hasClash) {
    parts.push("你们的合盘在部分维度上存在冲克关系——这在专业八字中称为'动态张力结构'。冲不一定是坏事，很多长期稳定的关系恰恰建立在合理的张力之上。冲意味着两人之间存在显著差异：行事节奏不同、决策方式不同、或是在压力下的反应模式不同。这些差异在日常中可能表现为分歧，但它们同时也是彼此成长的催化剂——对方看见的是你自己看不到的盲区。");
    parts.push("");
    parts.push("处理冲克结构的核心原则有三条。第一，在情绪升温时暂停争论——冲的负面影响在情绪化时会被放大，约定一个'冷静窗口'（比如半小时），让双方都有空间回到理性状态。第二，将分歧归类：哪些是需要解决的真问题，哪些只是风格差异不需要较真。第三，定期进行'关系复盘'——不是翻旧账，而是站在'我们是一队的'这个基本认知上，一起回顾最近哪些互动方式有效、哪些可以调整。冲克结构最大的优势在于：它迫使双方必须成长，而共同成长正是长期关系最坚固的粘合剂。");
  } else {
    parts.push("你们的合盘整体呈现出一种'弹性结构'——没有极端的共振也没有显著的对冲，这意味着你们的关系不会被命理层面的强大力量推着走。相比于天然高度契合或明显冲克的伴侣，你们拥有更大的自由度和可塑性：关系的走向更多取决于双方后天的选择和经营，而非先天八字结构的限定。");
    parts.push("");
    parts.push("这种弹性结构在专业视角下并非'中庸'——它实际上是一种优势，因为它意味着你们可以主动塑造关系模式，而不被命理惯性左右。具体建议：双方可以定期讨论并设定关系的'短期目标'（比如接下来三个月希望在哪方面改善互动），通过小步迭代建立属于你们自己的相处节奏。命理分析提供的是底层能量地图，而你们才是真正驾驶这辆车的人——弹性结构给了你们最大的方向盘自由度。");
  }
