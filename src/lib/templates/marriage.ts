...
function getPairReading(r: MarriageResult): string {
  ...
  const yuanTypeInsight: Record<string, string> = {
    "天赐良缘": "双方在日柱天干、五行生克和生肖三个核心维度上均呈吉象，先天匹配度较高。以下从六个维度展开，帮助理解这段关系的深层结构。",
    "互补良缘": "五行相生之势让这段关系具备「补其所缺」的潜力。互补型关系的优势在于，各自擅长的领域恰好是对方需要的支撑。以下从各维度展开。",
    "契合之缘": "多数维度中性偏吉，关系的发展更依赖于后天的经营而非先天的命理结构。这是一种需要在日常相处中逐步浮现其深度的关系类型。以下为各维度详解。",
    "成长之缘": "命理层面存在张力，但张力也是关系的催化剂。很多经历冲害磨合的关系，最终反而比一路平顺的关系更稳固。以下为各维度详解。",
  };

  const dims = r.dimensions ?? [];
  return [
    `此合盘判定为「${r.yuanType}」。${yuanTypeInsight[r.yuanType] ?? ""}`,
    ...
    ...dims.map((d) => `${d.title}——${d.text}`),
    "以上为合盘的全维度解读。合盘看的是先天结构的匹配度，但每一段关系最终的走向，取决于两个人的意愿、沟通和共同成长。命理分析提供的是理解彼此的框架，而非对关系的定论。",
  ].join("\n\n");
}
